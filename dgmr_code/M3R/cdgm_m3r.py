...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
        self.cond_intro = nn.Conv2d(in_channels=img_channel_1, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=img_channel1, kernel_size=3, padding=1, stride=1, groups=1,
                                bias=True)
    ...
    def forward(self, inp, gammas):
        ...
        #print(inp.shape)
        #x1, x2 = inp.chunk(6, dim=1)
        #print()		
        x1 = inp[:,:6,:,:] 
        x2 = inp[:,6:,:,:] 
        cond = x1 #torch.stack([x1, x2, x3], dim=1)
        b,c, h, w = cond.shape
        ...
        for encoder, down, cond_encoder, cond_down in zip(self.encoders, self.downs, self.cond_encoders, self.cond_downs):
            ...
            b, c, h, w = cond.shape
            #print(b, c, h, w,x.shape, cond.view(b//6, 6, c, h, w).sum(dim=1).shape)
            #tmp_cond = cond.view(b//6, 6, c, h, w).sum(dim=1)
            x = x + cond #tmp_cond
            encs.append(x)
            x = down(x)
            cond = cond_down(cond)
...
class CDGM(BaseNetwork):
    def __init__(self, ch,ch1,out, beta_schedule, **kwargs):
        super(CDGM, self).__init__(**kwargs)

        self.denoise_fn = UNet(img_channel=ch, img_channel_1=ch1, img_channel1=out)
...
class UNet1(nn.Module):
    ...
    def forward(self, co, inp, gammas):
        t = self.map(self.emb(gammas.view(-1, )))
        inp = self.check_image_size(inp)
        #print(inp.shape)
        #x1, x2 = inp.chunk(6, dim=1)
        ...
        x1 = inp[:,:6,:,:] 
        x2 = inp #inp[:,6:,:,:] 
        cond = co #torch.stack([x1, x2, x3], dim=1)
        b,c, h, w = cond.shape
        #cond = cond.view(b*n, c, h, w)
        #print(x1.shape, x2.shape)
        x = self.intro(x2)
...
class UNet2(nn.Module):
    ...
    def check_image_size(self, x):
        ...
        x = F.pad(x, (0, mod_pad_w, 0, mod_pad_h))
        return x
class CDGM_SMILE(BaseNetwork):
    def __init__(self, ch,ch1,out, beta_schedule, **kwargs):
        super(CDGM_SMILE, self).__init__(**kwargs)

        self.denoise_fn = UNet2(img_channel=ch, img_channel_1=ch1, img_channel1=out)
        self.beta_schedule = beta_schedule
    ...
    def p_mean_variance(self, y_t, t, clip_denoised: bool, y_cond=None):
        ...
        # y_0_hat = self.predict_start_from_noise(
        #         y_t, t=t, noise=self.denoise_fn(torch.cat([y_cond, y_t], dim=1), noise_level))
        y_0_hat = self.denoise_fn(y_cond, y_t, noise_level)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None):
        ...
        if mask is not None:
            noise_hat = self.denoise_fn(y_cond, y_noisy*mask+(1.-mask)*y_0, sample_gammas)
            loss = self.loss_fn(mask*noise, mask*noise_hat)
        else:
            y_0_hat = self.denoise_fn(y_cond, y_noisy, sample_gammas)
            loss = self.loss_fn(y_0, y_0_hat)
        return loss
