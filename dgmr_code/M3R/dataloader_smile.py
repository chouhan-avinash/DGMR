...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        [planet_cloudy_path, planet_cloudfree_path, s1_path,cloudmask_path] = self.filelist[index]
        ...
        if self.is_load_cloudmask:
            cm_data = get_opt_image(cloudmask_path) #np.load(cm_path, encoding='bytes', allow_pickle=True).astype('float32')
        ...
        cm_data = cm_data/255
        cm_data[ cm_data > 0] = 1#cm_data/255
        #print("#############", planet_cloudy_data.shape, s1_data.shape, cm_data.shape)
...
class ValDataset(Dataset):
    ...
    def __init__(self, opts, filelist):
        ...
        self.load_size = opts.load_size
        self.crop_size = opts.crop_size
        #self.f = open('/workspace/gpu8/my/work/M3R-CR/Align-CR/dat_list.txt', 'w+')
        if (self.is_load_SAR and not self.is_upsample_SAR) or (self.is_load_landcover and not self.is_upsample_landcover):
            self.crop_size_10m = self.crop_size*3/10
            assert self.crop_size_10m.is_integer()
            self.crop_size_10m = int(self.crop_size_10m)
    ...
    def __getitem__(self, index):
        ...
        [planet_cloudy_path, planet_cloudfree_path, s1_path,cloudmask_path] = self.filelist[index]
        #print(planet_cloudfree_path, planet_cloudy_path, S1_path, landcover_path, cloudmask_path)
        #stri = str(index)+ " " + planet_cloudfree_path+ " " + planet_cloudy_path+ " " + s1_path+ " " +landcover_path+ " " + cloudmask_path+ " \n" 
        #self.f.write(stri)
        #f.close()
        '''planet_cloudfree_path = os.path.join(self.input_data_folder, planet_cloudfree_path)
        planet_cloudy_path = os.path.join(self.input_data_folder, planet_cloudy_path)
