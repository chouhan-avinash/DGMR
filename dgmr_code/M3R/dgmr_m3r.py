...
class NonLocalAttention(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=3, scale=3, stride=1, softmax_scale=10, average=True, conv=default_conv):
...
class ResBlock_A(nn.Module):
    def __init__(self,in_channels,out_channels=256,alpha=0.1):
        ...
        self.tr = WAB(dim=out_channels ,input_resolution=[160,160],num_heads=4,window_size=8,shift_size=8 // 2)
    def forward(self,x):
        out =self.net(x)
        out = self.tr(out) #+ out
        return out
...
class MRNet(nn.Module):
    def __init__(self,in_channels,out_channels,alpha=0.1,num_layers = 16 , feature_sizes = 256):
        ...
        #m.append(ResBlock_A(feature_sizes,feature_sizes,alpha))
        self.csa = ResBlock_A(feature_sizes,feature_sizes,alpha)
        self.fin_org = nn.Conv2d(feature_sizes,4,kernel_size=3, bias=True,stride=1,padding=1)
        #self.fin_join = nn.Conv2d(feature_sizes,15,kernel_size=3, bias=True,stride=1,padding=1)
        self.fin_sr = nn.Conv2d(feature_sizes,2,kernel_size=3, bias=True,stride=1,padding=1)
        self.feat = nn.Conv2d(feature_sizes,4,kernel_size=3, bias=True,stride=1,padding=1)
        self.net = nn.Sequential(*m)
        ...
        self.opt_sa = WAB(dim=4 ,input_resolution=[160,160],num_heads=1,window_size=8,shift_size=8 // 2) #NonLocalAttention(channel=13, reduction=2)
        self.sar_sa = WAB(dim=2 ,input_resolution=[160,160],num_heads=1,window_size=8,shift_size=8 // 2) # NonLocalAttention(channel=2, reduction=1)


    
    def forward(self, x):
        B, C, H, W = x.shape
        x1_opt, x1_sar = x[:, :4, :, :], x[:, 4:6, :, :]			
        op1 = self.net(x)
        op1 = self.csa(op1) + op1
        op11 = self.rec_or(op1)
        op2 = self.fin_org(op11) #+ x[:,2:,:,:]
        op2_up = self.opt_sa(op2) +  x1_opt #.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) 
        		
		
        op2_up = op2_up #+ op2
        recon = self.rec(op1 ) 
        recon1= self.fin_sr(recon)#+ x[:,:2,:,:]
...
class LaplacianPyramidLoss(torch.nn.Module):
    def __init__(self, max_levels=3, channels=4, kernel_size=5, sigma=1, device=torch.device('cpu'), dtype=torch.float):
...
class DGMR(ModelBase):
    def __init__(self, opts):
        ...
        self.net_G = MRNet(6,4,1.0,16,256).cuda()
        ...
        beta_schedule = {"train": {"schedule": "sigmoid","n_timestep": 2000,"linear_start": 1e-06,"linear_end": 0.01},
                        "test": {
                            ...
                            "linear_start": 0.0001,
                            "linear_end": 0.09}}
        self.diffusion = CDGM(4,6, 4, beta_schedule)
        ...
        self.loss_fn= LaplacianPyramidLoss()	 #nn.L1Loss()
        self.l_sar = LaplacianPyramidLoss(channels=2)
    ...
    def set_input(self, _input):
        ...
        self.cloud_mask = inputs['cloudmask_data'].cuda()
    ...
    def optimize_parameters(self,epoch):
        ...
        torch.unsqueeze(cloud_m,1)),dim=1)
        ...
        self.loss_G1 = self.loss_fn(pred_Cloudfree_data, index_exp)  +self.l_sar(sar_data_p*cloud_m_stack,sar_or*cloud_m_stack)*0.5 +(ssim_loss+ssim_loss1*0.5)*.6 + nn.L1Loss()(diff1,diff2)*0.01 + ffl(pred_Cloudfree_data, index_exp) + ffl(sar_data_p*cloud_m_stack,sar_or*cloud_m_stack)*0.5
        ...
        data_diff = normalize_to_01_channelwise(data_diff)
        index_exp_down = normalize_to_01_channelwise(index_exp_down)		


        loss_diff = self.diffusion(y_0 = index_exp_down, y_cond = data_diff)

        self.loss_G = self.loss_G1 + loss_diff.mean()*.1 
    ...
    def val_scores(self):
        self.pred, b,c = self.forward() ##
        scores = {'PSNR': PSNR(self.pred, self.cloudfree_data),
                  'SSIM': SSIM(self.pred, self.cloudfree_data),
                  'SAM': SAM(self.pred, self.cloudfree_data),
                  'MAE': MAE(self.pred, self.cloudfree_data),
                  }
        return scores
