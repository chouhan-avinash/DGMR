...
from cdgm_m3r import CDGM_SMILE
from metrics import *
...
class ResBlock_A(nn.Module):
    def __init__(self,in_channels,out_channels=256,alpha=0.1):
        ...
        self.tr = WAB(dim=out_channels ,input_resolution=[256,256],num_heads=4,window_size=8,shift_size=8 // 2)
    def forward(self,x):
        out =self.net(x)
        out = self.tr(out) #+ out
...
class MRNet(nn.Module):
    def __init__(self,in_channels,out_channels,alpha=0.1,num_layers = 16 , feature_sizes = 256):
        ...
        self.fin_org = nn.Conv2d(feature_sizes,6,kernel_size=3, bias=True,stride=1,padding=1)
        
        self.fin_sr = nn.Conv2d(feature_sizes,2,kernel_size=3, bias=True,stride=1,padding=1)
        self.feat = nn.Conv2d(feature_sizes,6,kernel_size=3, bias=True,stride=1,padding=1)
        self.net = nn.Sequential(*m)
        ...
        self.opt_sa = WAB(dim=6 ,input_resolution=[256,256],num_heads=1,window_size=8,shift_size=8 // 2) #NonLocalAttention(channel=13, reduction=2)
        self.sar_sa = WAB(dim=2 ,input_resolution=[256,256],num_heads=1,window_size=8,shift_size=8 // 2) # NonLocalAttention(channel=2, reduction=1)

        
    
    def forward(self, x):
        B, C, H, W = x.shape
        x1_opt, x1_sar = x[:, :6, :, :], x[:, 6:8, :, :]			
        op1 = self.net(x)
...
class LaplacianPyramidLoss(torch.nn.Module):
    def __init__(self, max_levels=3, channels=6, kernel_size=5, sigma=1, device=torch.device('cpu'), dtype=torch.float):
...
class DGMR(ModelBase):
    def __init__(self, opts):
        ...
        self.net_G = MRNet(8,6,1.0,16,128).cuda()
        ...
        beta_schedule = {"train": {"schedule": "sigmoid","n_timestep": 2000,"linear_start": 1e-06,"linear_end": 0.01},
                        "test": {
                            ...
                            "linear_start": 0.0001,
                            "linear_end": 0.09}}
        self.diffusion = CDGM_SMILE(6,8, 6, beta_schedule)
    ...
    def optimize_parameters(self,epoch):
        ...
        cloud_m_s2 = torch.cat((cloud_m,cloud_m,cloud_m,cloud_m,cloud_m,cloud_m),dim=1)
        ...
        cloud_m_stack= torch.cat((cloud_m,cloud_m),dim=1)
        ...
        srm = cloud_m.cuda()	
        loss_diff = self.diffusion(y_0 = index_exp_down, y_cond = data_diff)#, mask = srm)
