...
class SEN12MSCRTS(Dataset):
    ...
    def __getitem__(self, pdx):  # get the time series of one patch
        ...
        #masks           = [self.lp(img) for img in masks]
        #mask            =  get_cloud_cloudshadow_mask(s2_cloudy, 0.2)
        #mask[mask != 0] = 1 #None if not self.cloud_masks else get_cloud_map(s2_cloudy, self.cloud_masks, self.cloud_detector)
...
class SEN12MSCR_train(Dataset):
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        s2_cloudy =  process_MS(s2_cloudy, self.method)
        s2 =  process_MS(s2, self.method)		
        
        s1_data = torch.from_numpy(s1)
        s2_cloudfree_data = torch.from_numpy(s2)
        s2_cloudy_data = torch.from_numpy(s2_cloudy)
        #if self.opts.is_use_cloudmask:
        cloud_mask = torch.from_numpy(mask)
		
		
        load_size = 256
        crop_size = 256  ##############here to be change
        y = random.randint(0, np.maximum(0, load_size - crop_size))
        x = random.randint(0, np.maximum(0, load_size - crop_size))		

        s1_data = s1_data[...,y:y+crop_size,x:x+crop_size]
        s2_cloudfree_data = s2_cloudfree_data[...,y:y+crop_size,x:x+crop_size]
        s2_cloudy_data = s2_cloudy_data[...,y:y+crop_size,x:x+crop_size]
        cloud_mask = cloud_mask[y:y+crop_size,x:x+crop_size]

        sample = {'input': {'S1': s1_data,
                            'S2': s2_cloudy_data, #process_MS(s2_cloudy, self.method),
                            'masks': cloud_mask,
                            'coverage': np.mean(mask),
                            'S1 path': os.path.join(self.paths[pdx]['S1']),
                            'S2 path': os.path.join(self.paths[pdx]['S2_cloudy']),
                            'coord': coord,
                            },
                    'target': {'S2': s2_cloudfree_data, #process_MS(s2, self.method),
                               'S2 path': os.path.join( self.paths[pdx]['S2']),
                               'coord': coord,
                                },
                    }
        #print(sample.keys(), sample['input'].keys())			
        return sample
